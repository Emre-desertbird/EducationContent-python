...
## Schritt 1
Ändere den letzten Namen in der Liste `Katzennamen` zu **"Shadow"**. Passe `player.say()` so an, dass der **letzte** Name der Liste im Chat erscheint. Drücke dann **„Katze auswählen“** und wähle die Katze mit diesem Namen.
### ~ tutorialhint 
...
## Schritt 2
Sortiere die Katzennamen mit `sort()` alphabetisch. Passe `player.say()` so an, dass der **vierte** Name im Chat erscheint. Achtung: Python zählt ab `0`, deshalb ist `Katzennamen[4]` nicht der vierte Eintrag. Drücke wieder **„Katze auswählen“** und wähle die passende Katze.
### ~ tutorialhint 
...
```template
Katzennamen = ["Smokey", "Oreo", "Sammy", "Patch", "Princess", "Snowy"]
